    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['vaccine:sign:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['vaccine:sign:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listSign, getSign, delSign, addSign, updateSign } from "@/api/vaccine/sign";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除接种签到编号为"' + ids + '"的数据项？').then(() => {
        this.loading = true;
        return delSign(ids);
      }).then(() => {
        this.loading = false;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('vaccine/sign/export', {
        ...this.queryParams
