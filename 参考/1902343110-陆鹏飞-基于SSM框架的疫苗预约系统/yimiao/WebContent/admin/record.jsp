...
<body>
  <section id="container">
      ...
      <section class="wrapper">
        ...
        <div class="col-md-12">
          <div class="content-panel">
            <h4>
              <i class="fa fa-angle-right"></i> 接种记录
              <form action="" class="pull-right mail-src-position form-inline" method="post">
              	<input type="hidden" name="page" id="page"/>
                <div class="input-append">
                 <input type="text" class="form-control" value="${doctor }" name="doctor" placeholder="医生">
    ...
    <!--main content end-->
  	...
  	<div aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" id="recordModal" class="modal fade">
          <div class="modal-dialog">
            <div class="modal-content">
              <form class="form-horizontal" action="" method="post" id="recordForm">
              <input type="hidden" id="id" name="id"/>
              <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title">请编辑记录信息</h4>
              </div>
  ...
  function update(id){
	  $.getJSON("<%=Const.QIAN %>record/json",{"id":id},function(data){
		  $("#id").val(id);
  ...
  $("#recordForm").submit(function(){
	  $.getJSON("<%=Const.QIAN %>record/update",$("#recordForm").serialize(),function(data){
		  if(data.code=="0"){
				location.href="<%=Const.QIAN %>record/list";
			}else{
  			alert(data.msg);
