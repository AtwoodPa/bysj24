...
<body>
      ...
      <section class="wrapper">
        ...
        <div class="col-md-12">
          <div class="content-panel">
            <h4>
              <i class="fa fa-angle-right"></i> 疫苗类型管理  <button type="button" onclick="add()" class="btn btn-success"><i class="fa fa-plus"></i> 添加</button>
  ...
  $("#addForm").submit(function(){
	  	$.getJSON("<%=Const.QIAN %>yimiaotype/add",$("#addForm").serialize(),function(data){
	  		if(data.code=="0"){
	  			location.href="<%=Const.QIAN %>yimiaotype/list";
	  		}else{
	  			alert(data.msg);
	  		}
	  	});
  ...
  //修改
  function update(id){
	 $.getJSON("<%=Const.QIAN %>yimiaotype/json",{"id":id},function(data){
  		$("#u_id").val(data.id);
  ...
  $("#updateForm").submit(function(){
  	$.getJSON("<%=Const.QIAN %>yimiaotype/update",$("#updateForm").serialize(),function(data){
  		if(data.code=="0"){
  			location.href="<%=Const.QIAN %>yimiaotype/list";
  		}else{
  			alert(data.msg);
  ...
  function del(id){
	  if(confirm('是否删除?')){
		  $.getJSON("<%=Const.QIAN %>yimiaotype/delete",{"id":id},function(data){
	    		if(data.code=="0"){
	    			location.href="<%=Const.QIAN %>yimiaotype/list";
	    		}else{
	      			alert(data.msg);
