  ...
  <div >
    ...
    <div class="article-page">
      ...
      <div class="article-meta">
        <span class="left">发布人: {{ article.author }}</span>
        <span class="right">{{ article.createTime }}</span>
      </div>

      <div style="border: 1px solid #ccc;height: 90%;margin-top: 10px" v-html="article.content"></div>
    </div>
  </div>

</template>

<script setup name="AlumniNewsDetail">
import { ref, onMounted } from 'vue';
import { getNews } from "@/api/core/news";


import { useRoute } from 'vue-router';
const route = useRoute();// 操作url
// 实例化路由
const router = useRouter();// 历史导航
